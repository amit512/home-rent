...
if (isset($_POST['login'])) {
    ...
    try {
        ...
        if ($data == false) {
			...
			$_SESSION['id'] = $data['id'];  // User ID from the database
                $_SESSION['mobile'] = $data['mobile'];  // Mobile from the database 
            if (md5($password) == $data['password']) {
                ...
                if ($data['kyc_status'] == 'Pending') {
                    ...
                    $_SESSION['kyc_status'] = $data['kyc_status'];
                    header('Location: kyc.php');
                    exit;
                } elseif ($data['kyc_status'] == 'Rejected') {
                    $errMsg = 'Your KYC verification was rejected. Please update your details.';
                } else {
                    // KYC is verified, proceed to dashboard
                    $_SESSION['id'] = $data['id'];
                    $_SESSION['username'] = $data['username'];
                    $_SESSION['fullname'] = $data['fullname'];
                    $_SESSION['role'] = $data['role'];
